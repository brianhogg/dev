...
A command-line interface (CLI) for deployments. This packages provides a reusable set of tools to reduce redundant dev tasks and chores such as release publication, changelog maintenance, pot file generation, and etc...
...
This packages offers a command-line interfaces and exposes the `bh-dev` binary. It is recommended to add a shorthand script to access the binary by adding a new script to your `package.json` file:
...
List available commands by running: `bh-dev help`.

Get help with a specific command by running: `bh-dev help [command]`.
